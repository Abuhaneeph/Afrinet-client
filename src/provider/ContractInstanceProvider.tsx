...
export const CONTRACT_ADDRESSES = {
  swapAddress: '0x6a257B0a406eB762C105130314dB15B1a29AbC4e',
  priceFeedAddress: '0x2B2068a831e7C7B2Ac4D97Cd293F934d2625aB69',
...
const THIRDWEB_RPC_URL = `https://rpc.sepolia.mantle.xyz`;
...
export const ContractInstanceProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const isCorrectNetwork = activeChain?.id === MANTLE_SEPOLIA_CHAIN_ID;
        ...
        try {
          ...
          if (wallet.getChain && wallet.switchChain) {
            try {
              ...
              if (currentChain.id !== MANTLE_SEPOLIA_CHAIN_ID) {
          ...
          if (typeof window !== 'undefined' && window.ethereum) {
            ...
            if (Number(network.chainId) === MANTLE_SEPOLIA_CHAIN_ID) {
              ...
              console.warn('Wrong network detected:', network.chainId, 'expected:', MANTLE_SEPOLIA_CHAIN_ID);
              setNetworkError(`Wrong network. Expected chain ID: ${MANTLE_SEPOLIA_CHAIN_ID}, got: ${network.chainId}`);
            }
          }
  ...
  useEffect(() => {
    if (isConnected && activeChain?.id && activeChain.id !== MANTLE_SEPOLIA_CHAIN_ID) {
      setNetworkError(`Connected to wrong network. Please switch to Mantle Sepolia (Chain ID: ${MANTLE_SEPOLIA_CHAIN_ID}).`);
  ...
  const reconnectSigner = async (): Promise<void> => {
    try {
      ...
      if (!isCorrectNetwork) {
        throw new Error(`Wrong network. Please switch to chain ID: ${MANTLE_SEPOLIA_CHAIN_ID}`);
      }
